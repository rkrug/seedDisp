...
#+begin_src sh :results output :tangle ./postTangleScript.sh :var VER=(vc-working-revision (buffer-file-name)) :var STATE=(vc-state (or (buffer-file-name) org-current-export-file))
  sed -i s/MAINVERSION/$MAINVERSION/ ./DESCRIPTION
  ...
  sed -i s/MAINVERSION/$MAINVERSION/ ./seedDisp-package.R
...
#+begin_src R :eval nil :tangle ./seedDisp-package.R :shebang :padline no :no-expand :comments no
  ...
  ##' Type: \tab Package\cr
  ##' Version: \tab MAINVERSION.MINORVERSION.PATCHVERSION\cr
...
* DESCRIPTION
#+begin_src R :tangle ./DESCRIPTION :shebang :padline no :no-expand :comments no
  Package: seedDisp
  Type: Package
...
* NAMESPACE
#+begin_src R :tangle ./NAMESPACE :shebang :padline no :no-expand :comments no
  useDynLib(seedDisp)
  exportPattern("^[[:alpha:]]+")
#+end_src
