...
#+begin_src R  :eval nil :tangle ./pkg/R/shiftMByAgnps.R :no-expand
  ##' Shift cells in m by one cell usind the direction given in agnps
  ##' 
  ##' @usage shiftMByAgnps(m, agnps)
  ##' @name shiftMByAgnps
  ##' @title Determine formula for Deposit layer
  ##' 
  ##' @title Shift cells in matrix m in direction given in agnps matrix
  ##' @param m \code{matrix} to be "flown"
  ##' @param agnps \code{matrix} of same dim=ension as \code{m} containing agnps flow directions as returned from GRASS
  ##' @return 
  ##' @author Rainer M Krug
  ##' @export 
  ##' @callGraphPrimitives
  shiftMByAgnps <- function(m, agnps) {
    ## define vectors for shifting
    ## agnps 1,  2,  3,  4,  5,  6,  7,  8) 
    dx <- c( 0, -1, -1, -1,  0, +1, +1, +1) # first index in matrix
    dy <- c(+1, +1,  0, -1, -1, -1,  0, +1) # second index in matrix
    ## create index Matix
    im <- cbind(
                rep(1:nrow(m), each=ncol(m)),
                rep(1:ncol(m), nrow(m))
                )
    ## create shifted index matrix
    ims <- im
    ## calculate x-shift
    ims[,1] <- im[,1] + dx[agnps[im]]
    ## and correct for "out of matrix shifts"
    ims[,1][ims[,1] < 1      ] <- NA
    ims[,1][ims[,1] > nrow(m)] <- NA
    ## calculate y-shift
    ims[,2] <- im[,2] + dy[agnps[im]]
    ## and correct for "out of matrix shifts"
    ims[,2][ims[,2] < 1      ] <- NA
    ims[,2][ims[,2] > ncol(m)] <- NA
    ## remove all which have NA from im and ims
    sel <- !is.na(ims[,1]) & !is.na(ims[,2])
    im <- im[sel,]
    ims <- ims[sel,]
    ## build shifted matrix m
    ## TODO deal with duplicate shifts into cell!
    i <- 
    m.out <- m
    m.out[] <- NA
    m.out[im] <- m[ims]
